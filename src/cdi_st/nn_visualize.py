...
def load_ground_truth(path: str) -> dict:
    ...
    print(f"  Phase range:  [{d['phase_true'].min():.4f}, {d['phase_true'].max():.4f}] rad")
...
def find_strained_samples(data_dir: str, limit: int = 20):
    ...
    print(f"\n{'='*76}")
    print(f"  Samples with non-trivial phase (strain or dislocation)")
    print(f"{'='*76}")
    print(f"  {'ID':<6} {'Material':<12} {'Shape':<10} {'Strain':<18} {'Disloc':<8} {'hkl'}")
    print(f"  {'-'*70}")
    ...
    for m in metas:
        ...
        if strain != 'none' or disloc != 'none':
            sid = meta['sample_id']
            print(f"  {sid:<6d} {meta['material']:<12} {meta['shape']:<10} "
    ...
    print(f"    python nn_phase_retrieval.py \\")
    print(f"      --input {data_dir}/sample_XXXXX.npz --mode hybrid \\")
    print(f"      --output reconstruction_strained.npz")
    print(f"{'='*76}\n")
...
def fig_overview(recon: dict, save_path: str = None):
    ...
    N = recon['phase'].shape[0]; c = N // 2

    fig, axes = plt.subplots(1, 3, figsize=(15, 5))
    fig.suptitle('Reconstructed Crystal (Real Space — Central Slice)',
                 fontsize=13, y=1.02)

    im0 = axes[0].imshow(recon['amplitude'][:, :, c], cmap='hot',
    ...
    im1 = axes[1].imshow(phase_slice, cmap='twilight',
                         vmin=-np.pi, vmax=np.pi,
                         origin='lower', aspect='equal')
    axes[1].set_title('Phase  φ(r)  [global offset removed]', fontsize=11)
    axes[1].set_xlabel('x'); axes[1].set_ylabel('y')
    cb1 = plt.colorbar(im1, ax=axes[1], shrink=0.8,
                       ticks=[-np.pi, -np.pi/2, 0, np.pi/2, np.pi])
    cb1.set_ticklabels(['-π', '-π/2', '0', 'π/2', 'π'])

    im2 = axes[2].imshow(support[:, :, c], cmap='Blues',
...
def fig_diffraction_comparison(recon: dict, truth: dict, save_path: str = None):
    ...
    N = amp_recon_q.shape[0]; c = N // 2

    def log_n(x):
        return np.log10(x + 1e-5)

    fig, axes = plt.subplots(2, 3, figsize=(15, 9))
    fig.suptitle('Diffraction Pattern: Measured vs Reconstructed (reciprocal space)',
                 fontsize=12, y=0.99)

    slices = [('qx-qy', lambda a: a[:, :, c]),
              ('qx-qz', lambda a: a[:, c, :]),
              ('qy-qz', lambda a: a[c, :, :])]
    ...
    for col, (name, getter) in enumerate(slices):
        ...
        im0 = axes[0, col].imshow(t_sl, cmap='jet', origin='lower',
                                   vmin=vmin, vmax=vmax, aspect='equal')
        axes[0, col].set_title(f'Measured  ({name})', fontsize=10)
        axes[0, col].tick_params(labelsize=7)

        im1 = axes[1, col].imshow(r_sl, cmap='jet', origin='lower',
                                   vmin=vmin, vmax=vmax, aspect='equal')
    ...
    fig.text(0.5, 0.02, f'Global reciprocal-space R-factor = {r:.4f}',
...
def fig_phase_comparison(recon: dict, truth: dict, save_path: str = None):
    ...
    if common.sum() > 10:
        ...
        pr_vals = phase_recon[common]; pt_vals = phase_true[common]
        if pt_vals.std() < 1e-6:
        ...
        rmse = float('nan'); corr_str = "N/A"

    N = phase_recon.shape[0]; c = N // 2

    fig, axes = plt.subplots(1, 3, figsize=(15, 5))
    fig.suptitle(f'Phase Comparison  |  RMSE = {rmse:.3f} rad  |  correlation = {corr_str}',
                 fontsize=12)

    kw = dict(cmap='twilight', vmin=-np.pi, vmax=np.pi, origin='lower', aspect='equal')
    titles = ['Ground truth φ (offset removed)',
    ...
    if meta.get('strain_type', 'none') == 'none' \
       and meta.get('dislocation_type', 'none') == 'none':
        fig.text(0.5, 0.01,
                 ...
                 ha='center', fontsize=10, color='#f0883e', style='italic')
...
def main():
    ...
    parser.add_argument('--find_strained', action='store_true',
