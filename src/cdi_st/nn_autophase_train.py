...
def train_one_epoch(model, physics, loss_fn, loader, optimizer, scaler, device):
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        if (batch_idx + 1) % 20 == 0:
            print(
                f"    batch {batch_idx+1}/{len(loader)}  loss={running['total']/n:.5f}"
            )
...
def train(
    ...
    print(f"\n{'='*62}")
    print("  AutoPhaseNet3D Unsupervised Training")
    print(f"{'='*62}")
    ...
    print(
        f"  Regularizers:   support_smoothness={support_smoothness} tv_phase={tv_phase}"
    )

    # Dataset
    full_ds = UnsupervisedBCDIDataset(
        data_dir, augment=False, max_samples=max_samples, grid_size=grid_size
    )
    n_total = len(full_ds)
    ...
    train_ds.dataset = UnsupervisedBCDIDataset(
        data_dir, augment=True, max_samples=max_samples, grid_size=grid_size
    )
    ...
    loss_fn = UnsupervisedBCDILoss(
        support_smoothness=support_smoothness, tv_phase=tv_phase
    )
    ...
    log_file = open(
        log_path, "a" if (log_path.exists() and resume) else "w", newline=""
    )
    ...
    for epoch in range(start_epoch, epochs):
        ...
        train_losses = train_one_epoch(
            model, physics, loss_fn, train_loader, optimizer, scaler, device
        )
        ...
        print(
            f"  train: total={train_losses['total']:.5f}  mae={train_losses['mae_diff']:.5f}"
        )
        print(
            f"  val:   total={val_losses['total']:.5f}  mae={val_losses['mae_diff']:.5f}  ({elapsed:.1f}s)"
        )
