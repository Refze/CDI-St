...
from .nn_dataset import create_dataloaders
from .nn_phase_model import BCDIPhaseLoss, PhaseUNet3D, count_parameters
...
def train(
    ...
    print(f"\n{'='*60}")
    print("BCDI Phase U-Net Training")
    print(f"{'='*60}")
    ...
    for epoch in range(start_epoch, epochs):
        ...
        log_writer.writerow(
            [
                ...
                train_losses["total"],
                train_losses["mse"],
                train_losses.get("fft_consistency", 0),
                train_losses.get("smoothness", 0),
                val_losses["total"],
                val_losses["mse"],
                val_losses.get("fft_consistency", 0),
                val_losses.get("smoothness", 0),
                elapsed,
            ]
        )
        log_file.flush()
