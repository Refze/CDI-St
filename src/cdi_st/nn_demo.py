...
def step_generate(
    num_samples=50, grid_size=64, material=None, output_dir="./demo_data"
):
...
def step_train(
    data_dir="./demo_data", output_dir="./demo_checkpoints", epochs=10, batch_size=4
):
...
def step_reconstruct(test_sample_path, model_path="./demo_checkpoints/best_model.pt"):
    ...
    print(f"\n{'='*60}")
    print("  Quality comparison (against ground truth)")
    print(f"{'='*60}")
    print(
        f"{'Method':<15} {'Phase RMSE':>12} {'Phase corr':>12} {'R-factor':>10} {'Time':>8}"
    )
    print(f"{'-'*60}")
    ...
    for name, result in results.items():
        ...
        if mask.sum() > 0:
            ...
            # Phase correlation
            corr = np.corrcoef(result.phase[mask].ravel(), phase_true[mask].ravel())[
                0, 1
            ]
        else:
            rmse = float("nan")
...
def step_plot(results, test_sample_path, output_dir="./demo_results"):
    ...
    for name, result in results.items():
        if len(result.error_metric) > 1:
            ax.semilogy(
                result.error_metric,
                label=f"{name} ({result.elapsed_seconds:.1f}s)",
                color=colors.get(name, "#888"),
                linewidth=2,
            )
        else:
            ax.axhline(
                result.error_metric[0],
                linestyle="--",
...
def main():
    ...
    parser.add_argument(
        "--num_samples", type=int, default=50, help="Training samples to generate"
    )
    parser.add_argument("--grid_size", type=int, default=64, help="Grid size N")
    parser.add_argument("--epochs", type=int, default=10, help="Training epochs")
    parser.add_argument(
        "--material", type=str, default=None, help="Lock to specific material"
    )
    parser.add_argument(
        "--skip_training", action="store_true", help="Skip data generation and training"
    )
